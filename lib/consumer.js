...
Consumer.prototype.rebalance = function() {
    var self = this;
    ...
    .then(function() {
        self.handle.rebalance(self.expected_offsets);
        self.resume();
        return _.keys(self.expected_offsets);
    });
};
...
Consumer.prototype._rdkakfa_recv_callback = function(kmsgs) {
    ...
    if (unexpected > 0) {
        self.logger.error('skipping', unexpected, 'messages for unexpected partitions');
    }
